  ...
  const [characters, total] = await Promise.all([
    prisma.character.findMany({
      ...
      select: {
        ...
        age: true,
        gender: true,
        occupation: true, occupationEn: true,
        personality: true, personalityEn: true,
        background: true, backgroundEn: true,
        avatarEmoji: true,
        portraitUrl: true,
        portraitImages: true,
        usageCount: true,
...
marketplaceRouter.get('/leaderboard', async (_req: AuthRequest, res: Response): Promise<void> => {
  const [byUsage, byRating] = await Promise.all([
    prisma.character.findMany({
      ...
      take: 10,
      select: {
        id: true, name: true, nameEn: true, avatarEmoji: true,
        occupation: true, occupationEn: true, portraitUrl: true,
